...
var samsungAssetsPathForSvgGifVideo = 'err: ASSETS-NOT-ASSIGNED';
...
function replaceImgUrl(fullUrl, urlAfterDoubleSlashImages) { 
  const fileExtension = fullUrl.split('.')[1].toLowerCase()
  const fileDotExtension = `.${fullUrl.split('.')[1].toLowerCase()}`
  if (fileExtension == 'jpg' || fileExtension == 'jpeg' || fileExtension == 'png' ) {
    return `${samsungAssetsPath}${urlAfterDoubleSlashImages}${fileDotExtension}`    
  } else {
    return `${samsungAssetsPathForSvgGifVideo}${urlAfterDoubleSlashImages}${fileDotExtension}`
  }
}
...
gulp.task('inline', () => {
  ...
  return gulp.src('dist/**/*.html')
    ...
    .pipe($.if(samsungAssetsPath !== '/', $.stringReplace(/\/images\/(.*?)\.[a-z0-9]+/g, 
      (fullUrl, urlAfterDoubleSlashImages) => {
        return replaceImgUrl(fullUrl, urlAfterDoubleSlashImages) 
      }
    )))
    ...
    .pipe($.if(samsungAssetsPath !== '/', $.stringReplace(/\?\$ORIGIN\_PNG\$\?\$ORIGIN\_PNG\$/g, '?$ORIGIN_PNG$')))
    .pipe($.if(samsungAssetsPath !== '/', $.stringReplace(/\?\$ORIGIN\_JPG\$\?\$ORIGIN\_JPG\$/g, '?$ORIGIN_JPG$')))
    .pipe($.if(samsungAssetsPath !== '/', $.stringReplace(/\?\$ORIGIN\_GIF\$\?\$ORIGIN\_GIF\$/g, '?$ORIGIN_GIF$')))
    .pipe(gulp.dest('dist'));
});
        ...
        if (res.env === 'P5') {
          ...
          },
          {
            ...
            if (res.assets === 'P5') {
              ...
              samsungAssetsPathForSvgGifVideo = '//images.samsung.com/is/content/samsung/p5/fr' + res.url;
